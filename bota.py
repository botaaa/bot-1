...
def parse(html):
    ...
    for row in table.find_all('div',class_='a-description'):
        ...
        second = prices[0].text.strip().split()[1]
        if second == 'млн':
        	second = second.replace('млн','000000')
...
def main():  
	...
	while True:
		...
		if last_chat_text.lower()==command1:
			...
			krisha_bot.send_message(last_chat_id, "Все ваши запросы пишите с обратным слэшом (/) и на русском языке, все название что вы вводите должно совпадать с данными на крыше")
			...
			krisha_bot.send_message(last_chat_id,"ok")
			new_offset=last_update_id+1
			krisha_bot.get_updates(new_offset)
			last_update = krisha_bot.get_last_update()
			last_update_id = last_update['update_id']
			...
			if last_chat_text.lower()=='/все':
				lin = li + '&das[who]=1&das[checked]=1'
